...
class MainPage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tabLayout?.getTabAt(0)!!.icon!!.setColorFilter(resources.getColor(R.color.colorBlue), PorterDuff.Mode.SRC_IN)
        ...
        notificationBadgeDrawable.number = ++count
        tabLayout?.addOnTabSelectedListener(object : OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                when (tab.position) {
                    0 -> tabLayout?.getTabAt(0)!!.icon!!.setColorFilter(resources.getColor(R.color.colorBlue), PorterDuff.Mode.SRC_IN)
                    1 -> {
                        tabLayout?.getTabAt(1)!!.setIcon(R.drawable.ic_selected_notifications_24dp)
                        //notifyBadge.clear();
                        notificationBadgeDrawable.isVisible = false
                    }
                    2 -> tabLayout?.getTabAt(2)!!.icon!!.setColorFilter(resources.getColor(R.color.colorBlue), PorterDuff.Mode.SRC_IN)
                    else -> {
